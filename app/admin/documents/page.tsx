...
export default function VehicleListPage() {
  ...
  const loadVehicles = async (page = 1) => {
    ...
    try {
      ...
      // اگر مشتری عادی است، فقط موترهای خودش را ببیند
      if (!isAdmin && currentUser) {
        filteredData = filteredData.filter(v => v.customer_id === currentUser._id);
      }
      
      setVehicles(filteredData);
      setPagination(result.pagination || { page: 1, total: filteredData.length, pages: 1 });
    } catch (error) {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold text-[#1a4a6a] mb-8">
        {isAdmin ? t('menuRegisteredVehicles') : '🚗 موترهای من'}
      </h1>
              ...
              <div className="flex justify-between items-start flex-wrap gap-4">
                <div className="flex-1">
                  ...
                  <div className="grid md:grid-cols-2 gap-2 text-sm mb-3">
                    ...
                    {isAdmin && vehicle.customer_name && (
                      <div>
                        <span className="text-gray-500">👤 مشتری:</span>{' '}
                        <span className="font-semibold">{vehicle.customer_name}</span>
                      </div>
                    )}
                  </div>
